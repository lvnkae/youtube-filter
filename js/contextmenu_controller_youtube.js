...
class ContextMenuController_Youtube extends ContextMenuController {
    ...
    get_renderer_node(element) {
        // shorts例外(25/07対応)
        const sub_short_reel = HTMLUtil.search_parent_node(element, e=> {
        ...
        if (sub_short_reel != null) {
            const short_reel = HTMLUtil.search_parent_node(sub_short_reel, e=> {
        ...
        // fullscreen-reccomend(26/01時点では動画終了画面おすすめ含む)例外
        const fullscr_node = HTMLUtil.search_parent_node(element, e=> {
            return e.localName === 'a' &&
                   e.className != null &&
                   e.className.indexOf('ytp-modern-videowall-still') >= 0;
        });
        if (fullscr_node != null) {
            return fullscr_node;
        }
        // endscreen-content(動画終了画面おすすめ動画)例外
        return HTMLUtil.search_parent_node(element, e=> {
            return e.localName === 'a' &&
                   e.className != null &&
    ...
    on_commentmute(element) {
        const nd_channel_id = element.querySelector("a#author-text");
        if (nd_channel_id == null) {
            return false;
        }
        ...
        if (YoutubeUtil.is_handle_author(author)) {
            const handle = author[0];
            channel = this.channel_info_accessor.get_channel_name(handle);
            channel_id = this.channel_info_accessor.get_channel_id(handle);
        } else {
            channel_id = author[2];
        }
        if (channel_id == null) {
